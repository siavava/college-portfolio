...
.code-container
  .top-container
    span.filename(v-if="filename") {{ filename }}
    span.language(v-if="language") {{ language }}
    .copy-container
      button.copy-button(@click="copy(code)")
        svg.w-4.h-4(v-if="copied" fill="none" viewBox="0 0 24 24")
          title Check
          path(
